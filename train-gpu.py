...
def load_dataset_from_folders(data_dir: Path, dataset_label: str, progress_every: int = 1000) -> tuple[np.ndarray, np.ndarray]:
    ...
    print(f"{dataset_label}: {total_files} Dateien gefunden.")
    for idx, (digit_label, file_path) in enumerate(files_with_labels, start=1):
        ...
        if idx == 1 or idx % progress_every == 0 or idx == total_files:
            print(f"{dataset_label}: {idx}/{total_files} geladen.")
...
def train_model_gpu(
    ...
    print(f"Lade Trainingsdaten aus: {TRAIN_DATA_DIR}")
    x_train, y_train = load_dataset_from_folders(TRAIN_DATA_DIR, dataset_label="Training")
    print(f"Geladene Trainings-Samples: {len(y_train)}")

    print(f"Lade Testdaten aus: {TEST_DATA_DIR}")
    x_test, y_test = load_dataset_from_folders(TEST_DATA_DIR, dataset_label="Testing")
    print(f"Geladene Test-Samples: {len(y_test)}")
...
def main() -> None:
    ...
    if args.version is None:
        raise SystemExit("Bitte --version angeben, z. B. --version 4.1")
